...
use rpu_core::{
    Anchor, AnimationMode, BinaryOp, BundledProject, BytecodeOp, CompiledProject, Condition,
    DestroyTarget, DrawCommand, Expr, OpCode, RectNode, ResizeMode, RpuProject, SceneCamera,
    ScriptProperty, ScriptTarget, SpriteNode, TextAlign, TextNode, WindowConfig,
    apply_scene_layout,
};
use rpu_graphics::{SceneFrame, register_generated_rgba_texture};
#[cfg(any(
    target_arch = "wasm32",
    all(
...
use rpu_scenevm::{RpuSceneApp, RuntimeContext};
        ...
        for document in &compiled.parsed_scenes {
            for scene in &document.scenes {
                ...
                for sprite in &scene.sprites {
                    let entity = runtime_sprite_entity(sprite);
                    if entity.template {
                        templates.push(entity);
                    } else {
                        entities.push(entity);
...
fn runtime_sprite_entity(sprite: &SpriteNode) -> RuntimeEntity {
...
fn entity_collider_rect(entity: &RuntimeEntity) -> (f32, f32, f32, f32) {
    (entity.pos[0], entity.pos[1], entity.size[0], entity.size[1])
}
